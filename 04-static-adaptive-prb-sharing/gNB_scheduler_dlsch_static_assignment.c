...
/* Returns a stable UE "ID" for filtering purposes:
 * - prefer CU-UE-ID (secondary_ue) when available (F1),
 * - else fall back to connected_ue_list index. */
                                       ...
                                       int current_harq_pid)
{
  ...
  * this helps allocate retransmission when TDA changes (e.g. new nrOfSymbols > old nrOfSymbols) */
  ...
  /* Fixed PRB mode for retransmissions: constrain search to UE's fixed range */
  int num_conn = 0;
  for (int i = 0; i < MAX_MOBILES_PER_GNB + 1 && UE_list[i] != NULL; i++) num_conn++;
  int fixed_ue_idx = (num_conn >= 2 && bwp_info.bwpSize >= 80) ? get_fixed_dl_prb_ue_index(UE, UE_list) : -1;
  if (fixed_ue_idx == 0) {
    rbStart = bwp_info.bwpStart;
    rbStop = bwp_info.bwpStart + 24;     // 24
  } else if (fixed_ue_idx == 1) {
    rbStart = bwp_info.bwpStart + 25;   // 25
  ...
  *n_rb_sched -= new_sched.rbSize;
  ...
  for (int rb = rbStart; rb < rbStart + new_sched.rbSize; rb++)
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (harq_pid >= 0) {
      ...
      if (sch_ret)
        sch_ret = allocate_dl_retransmission(mac, pp_pdsch, &n_rb_sched[beam.idx], UE, UE_list, beam.idx, harq_pid);
      if (!sch_ret) {
  ...
  while (iterator->UE != NULL) {
    ...
    /* Custom fixed-PRB allocation: UE1 (CU-UE-ID 1) gets 25 PRBs (0-24),
     * UE2 (CU-UE-ID 2) gets 55 PRBs (25-79). Use num of connected UEs (not
     * numUE) so fixed allocation applies when only one UE has data this slot. */
    int num_conn = 0;
    for (int i = 0; i < MAX_MOBILES_PER_GNB + 1 && UE_list[i] != NULL; i++) num_conn++;
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    if (fixed_two_ue_mode && (fixed_ue_idx == 0 || fixed_ue_idx == 1)) {
      /* Fixed 25/55 PRB allocation: bypass nr_find_nb_rb and set directly */
...
void post_process_dlsch(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pdsch, NR_UE_info_t *UE, NR_sched_pdsch_t *sched_pdsch)
{
  ...
  if (harq->round != 0) { /* retransmission */
    ...
    /* store last DL NPRB (RBs) for MAC statistics logging */
    UE->mac_stats.DL_NPRB = sched_pdsch->rbSize;
