...
/* Static PRB allocation configuration for different UEs */
typedef struct {
  uint64_t ue_id; /* CU-UE-ID if available, otherwise RNTI */
  int static_prb_start;
  int static_prb_size;
  bool enabled;
} static_prb_config_t;

/* Static PRB configurations for two UEs
 * Note: rnti == 0 means unassigned; the first two UEs seen will be bound lazily.
 */
...
/* \brief Print static PRB configuration for debugging */
static void print_static_prb_config(void) {
  LOG_I(NR_MAC, "=== Static PRB Configuration ===\n");
  for (int i = 0; static_prb_configs[i].enabled; i++) {
    if (static_prb_configs[i].ue_id == 0) {
      LOG_I(NR_MAC, "Slot %d: UNASSIGNED, PRB Start: %d, PRB Size: %d\n",
            i,
            static_prb_configs[i].static_prb_start,
            static_prb_configs[i].static_prb_size);
    } else {
      LOG_I(NR_MAC, "Slot %d: UE-ID: 0x%lx, PRB Start: %d, PRB Size: %d\n",
            i,
            (unsigned long)static_prb_configs[i].ue_id,
            static_prb_configs[i].static_prb_start,
            static_prb_configs[i].static_prb_size);
    }
  }
...
/* \brief Get static PRB configuration for a UE.
 * Uses CU-UE-ID when available (stable across RNTI changes on reconnection).
 * CU-UE-ID 1 -> 25 PRBs (0-24), CU-UE-ID 2 -> 55 PRBs (25-79). */
static static_prb_config_t* get_static_prb_config(const NR_UE_info_t *UE) {
  /* Prefer CU-UE-ID for stable mapping across RNTI changes */
  if (du_exists_f1_ue_data(UE->rnti)) {
    int cu_id = du_get_f1_ue_data(UE->rnti).secondary_ue;
    if (cu_id == 1 && static_prb_configs[0].enabled)
      return &static_prb_configs[0];
    if (cu_id == 2 && static_prb_configs[1].enabled)
      return &static_prb_configs[1];
    return NULL;
  }
  /* Fallback: RNTI-based binding for non-F1 (monolithic) */
...
/* \brief Check if a PRB is within static allocation for a UE */
static __attribute__((unused)) bool is_prb_in_static_allocation(const NR_UE_info_t *UE, int prb_index) {
  static_prb_config_t* config = get_static_prb_config(UE);
  if (!config) return false;
  
  return (prb_index >= config->static_prb_start && 
          prb_index < config->static_prb_start + config->static_prb_size);
}

static void nr_mac_addmod_bearers_from_cellgroup(NR_UE_sched_ctrl_t *sched_ctrl,
                                                 const struct NR_CellGroupConfig__rlc_BearerToAddModList *l)
{
  if (!l)
    return;
  for (int i = 0; i < l->list.count; ++i) {
    const NR_RLC_BearerConfig_t *conf = l->list.array[i];
    if (!conf || !conf->mac_LogicalChannelConfig || !conf->mac_LogicalChannelConfig->ul_SpecificParameters)
      continue;
    ...
    (void)nr_mac_add_lcid(sched_ctrl, &c);
  }
}
                         ...
                         const uint16_t rssi)
{
  ...
  if (ra->cfra) {
    ...
    nr_mac_addmod_bearers_from_cellgroup(&UE->UE_sched_ctrl, UE->CellGroup->rlc_BearerToAddModList);
    int ss_type;
    ...
    if (crnti != 0) { // 3GPP TS 38.321 Table 7.1-1: RNTI values, RNTI 0x0000: N/A
      ...
      const uint16_t old_beam_index = old_UE->UE_beam_index;
      old_UE->UE_beam_index = UE->UE_beam_index;
      const bool new_beam = old_beam_index != old_UE->UE_beam_index;
                                       ...
                                       const NR_tda_info_t *tda_info)
{
  ...
  /* Constrain to fixed PRB range when UE has static config (CU-UE-ID 1 or 2) */
  static_prb_config_t *ul_static = get_static_prb_config(UE);
  int rbStop = bwpSize - 1;
  if (ul_static && bwpSize >= 80) {
    rbStart = ul_static->static_prb_start;
    rbStop = ul_static->static_prb_start + ul_static->static_prb_size - 1;
  }
  ...
  if (reuse_old_tda && nrOfLayers == retInfo->nrOfLayers) {
    ...
    while (rbStart <= rbStop && (rballoc_mask[rbStart + bwpStart] & slbitmap))
      rbStart++;
    if (rbStart + retInfo->rbSize > bwpSize || rbStart + retInfo->rbSize - 1 > rbStop) {
  ...
  for (int rb = 0; rb < new_sched.rbSize; rb++)
    rballoc_mask[new_sched.rbStart + bwpStart + rb] |= SL_to_bitmap(new_sched.tda_info.startSymbolIndex, new_sched.tda_info.nrOfSymbols);
  return true;
}
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  while (iterator->UE != NULL) {
    ...
    /* Count connected UEs (same condition as DL fixed mode) */
    int num_conn_ul = 0;
    for (int i = 0; i < MAX_MOBILES_PER_GNB + 1 && UE_list[i] != NULL; i++) num_conn_ul++;
    /* Static PRB: CU-UE-ID 1->25 PRBs, 2->55 PRBs. Require 2+ connected UEs like DL. */
    static_prb_config_t* static_config = (num_conn_ul >= 2 && bi.bwpSize >= 80) ? get_static_prb_config(iterator->UE) : NULL;
    if (static_config) {
      /* Use static PRB allocation */
      rbStart = static_config->static_prb_start;
      uint16_t available_rb = static_config->static_prb_size;
      
      /* Check if static PRBs are available (not already allocated) */
      ...
      if (!static_prbs_available) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] Static PRBs not available, skipping UE\n", 
              iterator->UE->rnti, frame, slot);
        reset_beam_status(&nrmac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, dci_beam.new_beam);
        reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
        iterator++;
      ...
      LOG_D(NR_MAC, "[UE %04x][%4d.%2d] Using static PRB allocation: start=%d, size=%d\n", 
    ...
    if (!static_config) {
      /* Dynamic allocation - find available contiguous PRBs */
      while (rbStart + available_rb < bi.bwpSize && !(rballoc_mask[rbStart + bi.bwpStart + available_rb] & slbitmap) && available_rb < max_rbSize)
        available_rb++;
    } else {
      /* Static allocation - use configured size */
      available_rb = static_config->static_prb_size;
    }
...
if (static_config) {
  /* Force fixed PRB size for statically bound UE */
  ...
  sched.tb_size = nr_compute_tbs(sched.Qm,
    ...
    /* Mark PRBs as used - respect static allocation boundaries */
    if (static_config) {
      /* For static allocation, only mark PRBs within the static range */
      int static_start = static_config->static_prb_start;
      int static_end = static_start + static_config->static_prb_size;
      int actual_start = max(static_start, sched.rbStart);
      int actual_end = min(static_end, sched.rbStart + sched.rbSize);
      
      for (int rb = actual_start; rb < actual_end; rb++) {
        rballoc_mask[rb + bi.bwpStart] |= slbitmap;
      }
      
      LOG_D(NR_MAC, "[UE %04x] Marked static PRBs %d-%d as used\n", 
                                        ...
                                        nr_beam_mode_t beam_mode)
{
  ...
  pusch_pdu->beamforming.prgs_list[0].dig_bf_interface_list[0].beam_idx = convert_to_fapi_beam(UE->UE_beam_index, beam_mode);
  /* TRANSFORM PRECODING --------------------------------------------------------*/
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, slot_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  /* Print static PRB configuration on first call */
  static bool config_printed = false;
  if (!config_printed) {
    print_static_prb_config();
    config_printed = true;
  }
