...
/* Configuration parameters for time-varying PRB allocation */
#define TIME_CYCLE_DURATION 20.0  // 20 seconds cycle (10s intervals)
#define MAX_TIME_CONFIGS 20       // Maximum time intervals per cycle

/* Time-based configuration structure */
typedef struct {
...
/* Dynamic PRB allocation configuration for different UEs */
typedef struct {
  uint64_t ue_id; /* CU-UE-ID if available, otherwise RNTI */
  time_config_t time_configs[MAX_TIME_CONFIGS];  // Fixed array for better performance
  int num_configs;                               // Number of time configurations
  int current_config_idx;                        // Current active configuration index
  int last_cycle;                                // Last processed cycle number
  int last_seen_cycle;                           // Last cycle in which this UE was seen bound
  bool enabled;
} dynamic_prb_config_t;

/* Time-varying PRB configurations with 20-second cycles (10-second intervals)
 * Note: ue_id == 0 means unassigned; the first two UEs seen will be bound lazily.
 */
static dynamic_prb_config_t dynamic_prb_configs[] = {
  // UE 1 configuration - alternating pattern: 20 PRBs → 40 PRBs (10s intervals)
  {
    .ue_id = 0,  // Will be bound to first UE
    .time_configs = {
      {0.0, 10.0, 0, 20},   // 20 PRBs from 0-10 seconds (PRBs 0-19)
      {10.0, 20.0, 0, 40},  // 40 PRBs from 10-20 seconds (PRBs 0-39)
      // At 20s, cycle resets to 0s automatically
    },
    .num_configs = 2,
    .current_config_idx = 0,
    .last_cycle = -1,  // Initialize to -1 to detect first cycle
    .last_seen_cycle = -1,
    .enabled = true
  },
  // UE 2 configuration - alternating pattern: 40 PRBs → 20 PRBs (10s intervals)
  {
    .ue_id = 0,  // Will be bound to second UE
    .time_configs = {
      {0.0, 10.0, 40, 40},  // 40 PRBs from 0-10 seconds (PRBs 40-79)
      {10.0, 20.0, 40, 20}, // 20 PRBs from 10-20 seconds (PRBs 40-59)
      // At 20s, cycle resets to 0s automatically
    },
    .num_configs = 2,
    .current_config_idx = 0,
...
/* Get current simulation time in seconds with cyclic reset */
static double get_current_time_seconds(frame_t frame, slot_t slot) {
  // For numerology 1 (30kHz SCS): 1ms = 30 slots, 1 slot = 1/30 ms
  const double slot_duration_ms = 1.0 / 30.0;  // ms per slot
  double absolute_time = (frame * 20.0 + slot * slot_duration_ms) / 1000.0;  // Convert to seconds
  
  // Apply cyclic reset every TIME_CYCLE_DURATION seconds
  double cyclic_time = fmod(absolute_time, TIME_CYCLE_DURATION);
  
  // Debug logging every 50 frames to track timing more frequently
...
/* Get cycle number for logging/debugging */
static int get_current_cycle(frame_t frame, slot_t slot) {
  const double slot_duration_ms = 1.0 / 30.0;
  double absolute_time = (frame * 20.0 + slot * slot_duration_ms) / 1000.0;
  return (int)(absolute_time / TIME_CYCLE_DURATION);
}

/* Update configuration based on current cyclic time */
static void update_time_based_config(dynamic_prb_config_t *config, double cyclic_time, int current_cycle) {
  if (!config || !config->enabled || config->num_configs == 0) 
    return;
    
  // Check if we've entered a new cycle
  if (config->last_cycle != current_cycle) {
    if (config->last_cycle >= 0) {  // Not the first cycle
      LOG_I(NR_MAC, "UE-ID 0x%lx: Starting new 20s cycle %d (time reset to 0)\n",
            config->ue_id, current_cycle);
    }
    config->last_cycle = current_cycle;
    config->current_config_idx = 0;  // Reset to first configuration in new cycle
  }
    
  // Find the appropriate configuration for current cyclic time
  int new_config_idx = -1;
  for (int i = 0; i < config->num_configs; i++) {
    if (cyclic_time >= config->time_configs[i].start_time && 
        cyclic_time < config->time_configs[i].end_time) {
      new_config_idx = i;
      break;
    }
  }
  
  if (new_config_idx >= 0 && config->current_config_idx != new_config_idx) {
    config->current_config_idx = new_config_idx;
    LOG_I(NR_MAC, "UE-ID 0x%lx: Cycle %d, switching to config %d at time %.2fs (PRB start=%d, size=%d)\n",
          config->ue_id, current_cycle, new_config_idx, cyclic_time,
          config->time_configs[new_config_idx].prb_start,
          config->time_configs[new_config_idx].prb_size);
  } else if (new_config_idx < 0) {
    // Handle case where current time is outside all configured ranges within the cycle
    LOG_W(NR_MAC, "UE-ID 0x%lx: Cycle %d, time %.2fs outside configured ranges\n", 
          config->ue_id, current_cycle, cyclic_time);
  }
}

/* \brief Print dynamic PRB configuration for debugging */
static void print_dynamic_prb_config(frame_t frame, slot_t slot) {
  double cyclic_time = get_current_time_seconds(frame, slot);
  int current_cycle = get_current_cycle(frame, slot);
  
  LOG_I(NR_MAC, "=== Dynamic PRB Configuration ===\n");
  LOG_I(NR_MAC, "Current Cycle: %d, Cyclic Time: %.3fs\n", current_cycle, cyclic_time);
  
  for (int i = 0; dynamic_prb_configs[i].enabled; i++) {
    if (dynamic_prb_configs[i].ue_id == 0) {
      LOG_I(NR_MAC, "Config %d: UNASSIGNED\n", i);
      for (int j = 0; j < dynamic_prb_configs[i].num_configs; j++) {
        LOG_I(NR_MAC, "  Time [%.1f-%.1f]s: PRB Start: %d, PRB Size: %d\n",
              dynamic_prb_configs[i].time_configs[j].start_time,
              dynamic_prb_configs[i].time_configs[j].end_time,
              dynamic_prb_configs[i].time_configs[j].prb_start,
              dynamic_prb_configs[i].time_configs[j].prb_size);
      }
    } else {
      int cfg_idx = dynamic_prb_configs[i].current_config_idx;
      LOG_I(NR_MAC, "Config %d: UE-ID: 0x%lx, Active Config: %d\n",
            i, (unsigned long)dynamic_prb_configs[i].ue_id, cfg_idx);
      if (cfg_idx >= 0 && cfg_idx < dynamic_prb_configs[i].num_configs) {
        time_config_t *current = &dynamic_prb_configs[i].time_configs[cfg_idx];
        LOG_I(NR_MAC, "  Current: PRB Start: %d, PRB Size: %d\n",
              current->prb_start, current->prb_size);
      }
      for (int j = 0; j < dynamic_prb_configs[i].num_configs; j++) {
        LOG_I(NR_MAC, "  Time [%.1f-%.1f]s: PRB Start: %d, PRB Size: %d\n",
              dynamic_prb_configs[i].time_configs[j].start_time,
...
/* Get dynamic PRB configuration for a UE with cyclic time */
static dynamic_prb_config_t* get_dynamic_prb_config(const NR_UE_info_t *UE, 
                                                    frame_t frame, slot_t slot) {
  uint64_t key = (uint64_t)UE->rnti;
  double cyclic_time = get_current_time_seconds(frame, slot);
  int current_cycle = get_current_cycle(frame, slot);
  
  /* Reclaim stale bindings from previous cycles so reconnecting UEs can rebind */
  for (int i = 0; dynamic_prb_configs[i].enabled; i++) {
    dynamic_prb_config_t *cfg = &dynamic_prb_configs[i];
    if (cfg->ue_id != 0 && cfg->last_seen_cycle >= 0 && cfg->last_seen_cycle != current_cycle) {
      LOG_I(NR_MAC, "Releasing stale dynamic PRB binding (UE-ID 0x%lx) from cycle %d (current %d)\n",
            cfg->ue_id, cfg->last_seen_cycle, current_cycle);
      cfg->ue_id = 0;
      cfg->current_config_idx = 0;
      cfg->last_cycle = -1;
      cfg->last_seen_cycle = -1;
    }
  }

  /* First, try to find an existing binding */
  for (int i = 0; dynamic_prb_configs[i].enabled; i++) {
    if (dynamic_prb_configs[i].ue_id == key) {
      update_time_based_config(&dynamic_prb_configs[i], cyclic_time, current_cycle);
      dynamic_prb_configs[i].last_seen_cycle = current_cycle;
      return &dynamic_prb_configs[i];
    }
  }
  
  /* Otherwise, bind the UE to the first free slot */
  for (int i = 0; dynamic_prb_configs[i].enabled; i++) {
    if (dynamic_prb_configs[i].ue_id == 0) {
      dynamic_prb_configs[i].ue_id = key;
      dynamic_prb_configs[i].last_cycle = current_cycle;
      dynamic_prb_configs[i].last_seen_cycle = current_cycle;
      update_time_based_config(&dynamic_prb_configs[i], cyclic_time, current_cycle);
      LOG_I(NR_MAC, "Binding UE-ID 0x%lx to dynamic PRB config %d (RNTI 0x%04x) at cycle %d\n",
            key, i, UE->rnti, current_cycle);
      return &dynamic_prb_configs[i];
    }
  }
...
/* Get current PRB allocation for a UE with cycle information */
static bool get_current_prb_allocation(const NR_UE_info_t *UE, frame_t frame, slot_t slot,
                                      int *prb_start, int *prb_size) {
  dynamic_prb_config_t *config = get_dynamic_prb_config(UE, frame, slot);
  if (!config || config->current_config_idx >= config->num_configs || config->current_config_idx < 0) {
    LOG_D(NR_MAC, "[UE %04x] No dynamic config available (config=%p, idx=%d)\n", 
          UE->rnti, config, config ? config->current_config_idx : -1);
    return false;
  }
  
  time_config_t *current = &config->time_configs[config->current_config_idx];
  *prb_start = current->prb_start;
  *prb_size = current->prb_size;
  
  // Debug logging every 100 frames
  static int last_debug_frame = -1;
  if (frame % 100 == 0 && frame != last_debug_frame) {
    LOG_I(NR_MAC, "[UE %04x] Dynamic PRB: start=%d, size=%d, config_idx=%d\n", 
          UE->rnti, *prb_start, *prb_size, config->current_config_idx);
    last_debug_frame = frame;
  }
...
/* Check if a PRB is within dynamic allocation for a UE */
static bool is_prb_in_dynamic_allocation(const NR_UE_info_t *UE, frame_t frame, slot_t slot, int prb_index) {
  int prb_start, prb_size;
  if (!get_current_prb_allocation(UE, frame, slot, &prb_start, &prb_size)) {
    return false;
  }
  
  return (prb_index >= prb_start && prb_index < prb_start + prb_size);
}

/* Print current cycle status for debugging */
static void print_cycle_status(frame_t frame, slot_t slot) {
  double cyclic_time = get_current_time_seconds(frame, slot);
  int current_cycle = get_current_cycle(frame, slot);
  
  LOG_D(NR_MAC, "=== Cycle Status ===\n");
  LOG_D(NR_MAC, "Current Cycle: %d, Cyclic Time: %.3fs\n", current_cycle, cyclic_time);
  ...
  for (int i = 0; dynamic_prb_configs[i].enabled; i++) {
    if (dynamic_prb_configs[i].ue_id != 0) {
      ...
      if (cfg_idx >= 0 && cfg_idx < dynamic_prb_configs[i].num_configs) {
        time_config_t *current = &dynamic_prb_configs[i].time_configs[cfg_idx];
        LOG_D(NR_MAC, "UE-ID 0x%lx: Config %d, PRB start=%d, size=%d\n",
              dynamic_prb_configs[i].ue_id, cfg_idx, current->prb_start, current->prb_size);
      }
    }
  }
  LOG_D(NR_MAC, "==================\n");
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  while (iterator->UE != NULL) {
    ...
    /* Check if this UE has dynamic PRB allocation */
    int dynamic_prb_start = 0;
    int dynamic_prb_size = 0;
    bool has_dynamic_config = get_current_prb_allocation(iterator->UE, frame, slot, &dynamic_prb_start, &dynamic_prb_size);
    if (has_dynamic_config) {
      /* Use dynamic PRB allocation */
      rbStart = dynamic_prb_start;
      uint16_t available_rb = dynamic_prb_size;
      
      /* Check if dynamic PRBs are available (not already allocated) */
      ...
      if (!dynamic_prbs_available) {
        LOG_D(NR_MAC, "[UE %04x][%4d.%2d] Dynamic PRBs not available, skipping UE\n", 
              iterator->UE->rnti, frame, slot);
        reset_beam_status(&nrmac->beam_info, frame, slot, iterator->UE->UE_beam_index, slots_per_frame, dci_beam.new_beam);
        reset_beam_status(&nrmac->beam_info, sched_frame, sched_slot, iterator->UE->UE_beam_index, slots_per_frame, beam.new_beam);
        iterator++;
      ...
      LOG_D(NR_MAC, "[UE %04x][%4d.%2d] Using dynamic PRB allocation: start=%d, size=%d\n", 
            iterator->UE->rnti, frame, slot, rbStart, available_rb);
    } else {
    ...
    if (!has_dynamic_config) {
      /* No dynamic allocation - find available contiguous PRBs */
      while (rbStart + available_rb < bi.bwpSize && !(rballoc_mask[rbStart + bi.bwpStart + available_rb] & slbitmap) && available_rb < max_rbSize)
        available_rb++;
    } else {
      /* Dynamic allocation - use configured size */
      available_rb = dynamic_prb_size;
    }
...
if (has_dynamic_config) {
  /* Force fixed PRB size for dynamically configured UE */
  sched.rbSize = dynamic_prb_size;
  ...
  sched.tb_size = nr_compute_tbs(sched.Qm,
    ...
    /* Mark PRBs as used - respect dynamic allocation boundaries */
    if (has_dynamic_config) {
      /* For dynamic allocation, only mark PRBs within the dynamic range */
      int dynamic_start = dynamic_prb_start;
      int dynamic_end = dynamic_start + dynamic_prb_size;
      int actual_start = max(dynamic_start, sched.rbStart);
      int actual_end = min(dynamic_end, sched.rbStart + sched.rbSize);
      
      for (int rb = actual_start; rb < actual_end; rb++) {
        rballoc_mask[rb + bi.bwpStart] |= slbitmap;
      }
      
      LOG_D(NR_MAC, "[UE %04x] Marked dynamic PRBs %d-%d as used\n", 
            iterator->UE->rnti, actual_start, actual_end - 1);
    } else {
      /* Dynamic allocation - mark all allocated PRBs */
      for (int rb = 0; rb < sched.rbSize; rb++) {
        rballoc_mask[rb + sched.rbStart + bi.bwpStart] |= slbitmap;
      }
    }
...
void nr_schedule_ulsch(module_id_t module_id, frame_t frame, slot_t slot, nfapi_nr_ul_dci_request_t *ul_dci_req)
{
  ...
  /* Print dynamic PRB configuration on first call */
  static bool config_printed = false;
  if (!config_printed) {
    print_dynamic_prb_config(frame, slot);
    config_printed = true;
  }
